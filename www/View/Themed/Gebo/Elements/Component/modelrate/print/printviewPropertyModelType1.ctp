...
$model_date = str_replace(" ","&nbsp;",$model_date);
...
$properties = array_orderby($data, 'rank_id', SORT_ASC);
			...
			foreach($properties as $property){
				...
				$weaponnames = $property['weapon_name'];
				
					if($property['weapon_name']=="" && $property['weapon_id']!=""){
									
									$weapon = $this->requestAction(
									 array('controller' => 'ModelRates', 'action' => 'getWeaponName/' . $property['weapon_id'] . '/' . $property['id']),
									 array('return')
								  );
								  $weaponnames = $weapon;
				...
				if($chk==0){
					...
					<td class="center">'.$weaponnames.'</td>
		...
		<tr class="row_body">
			...
			<td colspan="4" align="right">' . $division['name'] . '</td>
			<td></td>
			<td ><b>รวม</b></td>
...
<tr class="row_body">
	...
	<td></td>
	<td><b>รวม</b></td>
	<td class="center"><b>'.number_format($sumRankGroup[3][0],0,".",",").'</b></td>
	<td class="center"><b>'.number_format($sumRankGroup[3][1],0,".",",").'</b></td>
	<td class="center"><b>'.number_format($sumRankGroup[3][2],0,".",",").'</b></td>
	<td class="center"><b>'.number_format($sumRankGroup[3][4],0,".",",").'</b></td>
	<td></td>
	<td></td>
</tr>
';
...
{
	$mpdf->debug=true;
	$mpdf->Output('/files/pdf/' . $model_code . ".pdf");
}
$mpdf->Output();
