    ...
    <div class="span12">
        <div class="box box-color box-bordered">
            <div class="box-content nopadding">
                <?php echo $this->Form->create('MissionCriticals', array('action' => 'save', 'class' => 'form-horizontal form-bordered')); ?>
                <div class="row-fluid">
                    <input type="hidden" name="data[MissionCriticals][id]" value="<?php echo $default['id']; ?>"/>                                    
                    <input type="hidden" name="data[MissionCriticals][action]" value="<?php echo $default['action']; ?>"/> 
                    <table class="table table-bordered">
                        <tbody>
                            <tr>
                                <td style="text-align:right;">* ชั้นความลับ : </td>
                                <td>
                                    <input name="data[MissionCriticals][secret]" type="radio" value="1" <?php echo (isset($default['secret']) && $default['secret'] == '1') ? "checked='checked'" : ""; ?>> ลับ 
                                    &nbsp;&nbsp;
                                    <input name="data[MissionCriticals][secret]" type="radio" value="0" <?php echo (isset($default['secret']) && $default['secret'] == '0') ? "checked='checked'" : ""; ?>> ไม่ลับ    
                                </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td>
                                    <select name="data[MissionCriticals][w_year]"  data-placeholder="" class="chzn_a input-xlarge" >
                            ...
                            <tr>
                                ...
                                <td>
                                    <select name="data[MissionCriticals][w_area_id]" onchange="changearea(this)" data-placeholder="" class="chzn_a input-xlarge" >
...
<!--                                    <span>
                            ...
                            <tr>
                                ...
                                <td>
                                    <textarea name="data[MissionCriticals][w_keyword]" id="wysiwg_mini" cols="1" rows="6" class="span12" placeholder=""><?php echo (isset($default) && isset($default['w_keyword'])) ? $default['w_keyword'] : ""; ?></textarea>
                                </td>
                            </tr> 
                            <tr>
...
<script type="text/javascript">
    ...
    function changearea(obj) {
        ...
        if (val == '999') {
            ...
            $('#tr_area_other').hide();
            $("input[name='data[MissionCriticals][w_area_other]']").val('');
        }
    }

    $("select[name='data[MissionCriticals][w_area_id]']").val('<?php echo (isset($default) && $default['w_area_id'] != '0') ? $default['w_area_id'] : '0'; ?>');
    ...
    function modalActionRemoveDocument(fileID) {
        if (confirm('ยืนยันการลบเอกสารแนบ')) {
            var url = "../../MissionCriticals/removeDocument/" + fileID;
            $.post(url, {
                file_id: fileID,
            }, function (data) {
    ...
    function modalAction2() {
        var form_action = $("input[name='data[MissionCriticals][action]']").val();
        var key_id = $("input[name='data[MissionCriticals][id]']").val();

        var secret = $("input:radio[name ='data[MissionCriticals][secret]']:checked").val();
        var w_name = $("input[name='data[MissionCriticals][w_name]']").val();
        var w_year = $("select[name='data[MissionCriticals][w_year]']").val();
        var w_area_id = $("select[name='data[MissionCriticals][w_area_id]']").val();
        var w_creater = $("input[name='data[MissionCriticals][w_creater]']").val();
        var w_keyword = $("textarea[name='data[MissionCriticals][w_keyword]']").val();
        var w_area_other = $("input[name='data[MissionCriticals][w_area_other]']").val();
...
//        }
        ...
        if (w_name != "")
        {
            var url = "../../MissionCriticals/save/" + key_id + "/" + form_action;
            $.post(url, {
                deleted: 'N',
                order_sort: 0,
                secret: secret,
                ...
                if (data.status == "SUCCESS") {
                    $(".close").trigger("click");
                    window.location = "../../MissionCriticals/index";
                } else {
    ...
    // File attach
    var id = $("input[name='data[MissionCriticals][id]']").val();
    var uploader = document.getElementById('uploader0');
    upclick(
            {
                element: uploader,
                action: '../../MissionCriticals/uploadDocument/' + id,
                onstart:
                        function (filename)
                        {
                ...
                oncomplete:
                        function (response_data)
                        {
                            ...
                            if (data[0].result === 'success') {
                                ...
                                document_link = '<div id="row_doc_key_' + document_id + '"><a href="/files/missioncriticals/' + document_name + '" target=_blank>';
