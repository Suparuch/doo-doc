...
class HMHistoryBooksController extends AppController {

    public $name = 'HMHistoryBooks';
    public $uses = array('HMHistoryBook', 'Unit', 'TableAttachFile');
    ...
    public function index() {
        ...
        if (empty($dataRequest)) {
            ...
            $whereConditions[] = "HMHistoryBook.secret = '0'";
            $conditions = array('limit' => $this->Generator->setLimit(),
                'order' => array('id' => 'desc'),
                'conditions' => $whereConditions
            );
            $total = $this->HMHistoryBook->find('count', $conditions);
            $HMHistoryBooks = $this->HMHistoryBook->find('all', $conditions);
        } else {
            ...
            if (!empty($dataRequest['txtHbName'])) {
                $whereConditions[] = '(HMHistoryBook.hb_name like \'%' . trim($dataRequest['txtHbName']) . '%\' )';
            }
            if (!empty($dataRequest['txtHbNo'])) {
                $whereConditions[] = '(HMHistoryBook.hb_no like \'%' . trim($dataRequest['txtHbNo']) . '%\' )';
            }
            if (!empty($dataRequest['txtHbAuthor'])) {
                $whereConditions[] = '(HMHistoryBook.hb_author like \'%' . trim($dataRequest['txtHbAuthor']) . '%\' )';
            }
            $secret = $dataRequest['rdbSecret'];
            $whereConditions[] = "HMHistoryBook.secret = '" . $secret . "'";
            ...
            // Query
            $total = $this->HMHistoryBook->find('count', array('conditions' => $whereConditions));
            $HMHistoryBooks = $this->HMHistoryBook->find('all', $conditions);
        }
        ...
        $breadcrumb = array(
            'controller' => array(
                ...
                'url' => '',
                'subtitle' => 'ระบบจัดเก็บเอกสารและหนังสือประวัติศาสตร์'
            ),
        );
        $this->set('breadcrumb', $breadcrumb);

        // Set model to view
        $this->set('HMHistoryBooks', $HMHistoryBooks);
        $pagination = array('offset' => $offset, 'total' => $total, 'limit' => $this->Generator->setLimit(), 'model' => 'HMHistoryBook', 'pages' => 5);
    ...
    public function form($id = null) {
        ...
        if (!empty($id)) {
            ...
            foreach ($HMHistoryBooks[0]['HMHistoryBook'] as $key => $value)
                $default[$key] = $value;
    ...
    public function save($id = null, $formAction = "EDIT") {
        ...
        if (!empty($data)) {
            if (!empty($id)) {
                $this->HMHistoryBook->id = $id;
                if ($formAction == "ADD") {
                    $data['id'] = $id;
                    $this->HMHistoryBook->create();
                }
                $status = $this->HMHistoryBook->save($data) ? 'SUCCESS' : 'FAILED';
            }
        } else
            $status = 'FAILED';
                    ...
                    if (is_uploaded_file($_FILES['Filedata']['tmp_name'])) {
                        if ($_FILES['Filedata']['error'] > 0) {
                            ...
                            if ($uploadSuccess) {
                                ...
                                $document['file_table_name'] = 'HMHistoryBooks';
