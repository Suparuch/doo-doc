...
function gotolist()
{
	location.href='/Document/outbox/<?php echo $type_id;?>';
}
</script>
    <table width="100%" cellpadding="5"><tr><td>
<button class='main_menu' onclick='gotolist()'><br />บันทึกข้อมูล<br /><br /></button>
</td>
<td>
<button class='main_menu' onclick='javascript:;'><br />รอหนังสือกลับ <br /><br /></button>
</td>
</tr>
</table>
<hr />
		...
		<div class="box box-bordered box-color">
			...
			<div class="row-fluid">
				...
				<div class="span8">
					<div class="dataTables_filter" id="dt_gal_filter">
						<label>
							<form action="../../Document/indexwaiting/<?php echo $type_id;?>" id='form_search' method="POST" class="form-horizontal form-bordered" enctype="multipart/form-data">
                            ...
                            <option value="P">ยังไม่รับ</option>
                             <option value="C">ส่งคืน</option>
								...
								<button id="DocumentFlow-submit" type="submit" class="btn btn-primary mm_13_list" style="display: inline-block;">ค้นหา</button>
			...
			<div class="box-content nopadding">
				...
				<table class="table table-bordered">
					<thead>
						<tr>
                                                        <th style="text-align: center;width:50px;display:none;"><input type="checkbox" class="toggle"></th>
                            <th style="text-align: center;width:50px;display:none;">ลำดับ</th>
                            
                            <th style="text-align: center;width:200px;display:none;">ชั้นความลับ</th>
                            <th style="text-align: center;width:200px;display:none;">ชั้นความเร่งด่วน</th>
                            <th style="text-align: center;width:150px;">เลขที่หนังสือ</th>
<th style="text-align: center;width:50px;">ลงวันที่</th>
							<th style="">ชื่อเรื่อง</th>
                            <th style="text-align: center;width:150px;display:none;">วันที่รับ</th>
                            <th style="text-align: center;width:150px;display:none;">ผู้รับ</th>
                                                        <th style="text-align: center;width:200px;">การกระทำ</th>
                                                        <th style="text-align: center;width:100px;display:none;">ไฟล์แนบ</th>
<th style="text-align: center;width:100px;">ยกเลิก</th>
						</tr>
					</thead>
					<tbody>
                    <?php
					...
					?>
							...
							<td style='display:none;'><?php echo isset($secrete[$doc['secrete']])?$secrete[$doc['secrete']]:"";?>
							
							</td>
                                                        
                            <td  style='display:none;'><?php echo isset($fast[$doc['fast']])?$fast[$doc['fast']]:"";?>
...
<td style='display:none;'><?php echo $doc['recieve_date'];?>
...
if($doc['status']!="9"){
?>
<a href="#myModal2" onclick="javascript: recievedoc('<?php echo $doc['id'] ;?>',<?php echo ++$doc['status'];?>);"  class="link-bb display" data-tableid="smpl_tbl">นำเรียนต่อ</a>
<a href="#myModal2" onclick="returnDocument('<?php echo $doc['id'] ;?>','<?php echo $doc['outtype'] ;?>');" data-toggle="modal" data-backdrop="static" class="link-bb display" data-tableid="smpl_tbl">รับหนังสือกลับ</a>
<?php 
}
...
?>
							...
							foreach($docs as $d){
								...
								if($doc['file_name']=="")
									continue;
								$i++;
							 echo "$i . <a href='/Document/download_file/" . $doc['id'] . "' target='_blank'/>" . $doc['file_name'] .  "</a>";
							  ?>
                             <a href=<?php echo "'/download2.php?path=files/documentflow/1/" . $doc['id']   . "&file_name=" . $doc['file_name'] . "'";?> target='_blank'><i class="icon-download-alt"></i></a><br />
                             <?php
							}
							?>
                            </td>
<td>
    <a href="#" onclick="javascript:canceldoc('<?php echo $doc['id'] ;?>');" data-toggle="modal" data-backdrop="static" class="link-bb display" data-tableid="smpl_tbl">ยกเลิก</a>
...
<script type="text/javascript">
		 ...
		 $('#customModal2').html('');
			...
			function returnDocument(id,objType){

		 $('#customModal2').html('');
				  $('#customModalHeader2').html('รายละเอียดการคืนหนังสือ');
				  $('#customModalAction2').html('บันทึก');
				  $('#customModal2').load("../../Document/returndocform/" + id + "/" + objType,function(data) {
						$('#customModal2').html(data);
				  });
...
function canceldoc(objId){
				 var url = "../../Document/cancelDoc";
						$.post(url,{
						id:objId
				},function(data){
