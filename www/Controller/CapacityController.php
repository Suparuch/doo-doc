...
class CapacityController extends AppController {

	public $name = 'Capacity';
	public $uses = array('Document','DocumentGroup','PdxReport');
	...
	$whereConditions['deleted'] = 'N';
			...
			$whereConditions[] = ' (parent_id=0  or parent_id is null)';
			$conditions = array('limit' => $this->Generator->setLimit(),'offset' => $offset,
								'order' =>  array('id' => 'desc'),
								'conditions' => $whereConditions
							   ); 
							
	 		//$pdxreport = $this->PdxReport->find('all',$conditions);  
			...
			foreach($pdxreportx as $row){
				...
				if(count($child)>0)
				{
					foreach($child as $row2){
						$r = $row2;
						$r['PdxReport']['unit'] = " - " . $r['PdxReport']['unit'];
						$pdxreport[] = $r;
					}
				}
				
			}     
			$total = $this->PdxReport->find('count',$whereConditions);
			...
			if(!empty($unit))
				$whereConditions[] = ' (unit ILIKE \'%'.$unit.'%\' OR unit ILIKE \'%'.$unit.'%\' ) ';
			if(!empty($code))
		...
		$this->set('PdxReport',$pdxreport); 
		$breadcrumb = array (   'controller' => array(   
								'name' => 'รายงานความพร้อมรบ' ,
								'url' => ''
								)
		);
