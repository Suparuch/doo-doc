...
class MissionCriticalsController extends AppController {

    public $name = 'MissionCriticals';
    public $uses = array('MissionCritical', 'Area', 'TableAttachFile');
    ...
    public function index() {
        ...
        if (empty($dataRequest)) {
            ...
            $whereConditions[] = "MissionCritical.secret = '0'";
            $conditions = array('limit' => $this->Generator->setLimit(),
                'order' => array('id' => 'desc'),
                'conditions' => $whereConditions
            );
            $total = $this->MissionCritical->find('count', $conditions);
            $MissionCriticals = $this->MissionCritical->find('all', $conditions);
        } else {
            ...
            $whereConditions[] = "MissionCritical.secret = '" . $secret . "'";
            if (!empty($dataRequest['txtWName'])) {
                $whereConditions[] = '(MissionCritical.w_name like \'%' . trim($dataRequest['txtWName']) . '%\' )';
            }
            if (!empty($dataRequest['ddlYear'])) {
                $whereConditions[] = '(MissionCritical.w_year = \'' . trim($dataRequest['ddlYear']) . '\' )';
            }
            if (!empty($dataRequest['ddlArea'])) {
                $whereConditions[] = '(MissionCritical.w_area_id = \'' . trim($dataRequest['ddlArea']) . '\' )';
            }
            if (!empty($dataRequest['txtWCreater'])) {
                $whereConditions[] = '(MissionCritical.w_creater like \'%' . trim($dataRequest['txtWCreater']) . '%\' )';
            }
...
//                $whereConditions[] = '(MissionCritical.w_create_date =\'' . $dStart . '\' )';
                $dCreateDate = $this->TextUtil->formatDateForWhereCond($dataRequest['dpkCreateDate']);
                if (!empty($dCreateDate)) {
                    $whereConditions[] = '(MissionCritical.w_create_date =\'' . $dCreateDate . '\' )';
                } else {
            ...
            // Query
            $total = $this->MissionCritical->find('count', array('conditions' => $whereConditions));
            $MissionCriticals = $this->MissionCritical->find('all', $conditions);
        }
        ...
        $this->set('MissionCriticals', $MissionCriticals);
        $pagination = array('offset' => $offset, 'total' => $total, 'limit' => $this->Generator->setLimit(), 'model' => 'MissionCritical', 'pages' => 5);
    ...
    public function form($id = null) {
        ...
        if (!empty($id)) {
            ...
            foreach ($MissionCriticals[0]['MissionCritical'] as $key => $value)
                $default[$key] = $value;
    ...
    public function save($id = null, $formAction = "EDIT") {
        ...
        if (!empty($data)) {
            if (!empty($id)) {
                $this->MissionCritical->id = $id;
                if ($formAction == "ADD") {
                    $data['id'] = $id;
                    $this->MissionCritical->create();
                }
                $status = $this->MissionCritical->save($data) ? 'SUCCESS' : 'FAILED';
            }
        } else
            $status = 'FAILED';
    ...
    public function delete() {
        ...
        foreach ($ids as $id) {
            if (!empty($id)) {
                ...
                $data['deleted'] = "Y";
                $this->MissionCritical->id = $id;
                if ($this->MissionCritical->exists())
                    $status = $this->MissionCritical->save($data) ? 'SUCCESS' : 'FAILED';
                else
                    $status = 'FAILED';
            } else
                    ...
                    if (is_uploaded_file($_FILES['Filedata']['tmp_name'])) {
                        if ($_FILES['Filedata']['error'] > 0) {
                            ...
                            if ($uploadSuccess) {
                                ...
                                $document['file_table_name'] = 'MissionCriticals';
