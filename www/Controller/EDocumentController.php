...
class EDocumentController extends AppController {

	public $name = 'EDocument';
	public $uses = array('Document','DocumentGroup','Corp','Unit');
	...
	public function index($corp_id=null) {
		...
		if(empty($dataRequest)){
			...
			$whereConditions[] = " (updated_by='$user_id' or updated_by is null ) ";
	 ...
	 $conditions = array('conditions' => array('deleted' => 'N'), 'order' => array('order_sort' => 'asc'), 'fields' => array('id', 'short_name'), 'cache' => 'longShortNameList', 'cacheConfig' => 'long');
		...
		$breadcrumb = array (   'controller' => array(   
								'name' => 'ระบบงานรับ - ส่ง  [ ' . $Units[$corp_id] .' ]'  ,
								'url' => ''
								)
		);
...
$DocumentGroup = $this->DocumentGroup->find('list',$conditions); 
	...
	public function save($corp_id = null,$id = null){
		...
		if(isset($_POST['data']) && count($data)<=0)
			$data = $_POST['data'];
	//	print_r($_FILES);
		//print_r($data);
	...
	public function download_file($id){
		...
		$whereConditions = array();
				...
				$file = $_SERVER['DOCUMENT_ROOT'] . 'app/webroot/files/edocument';
				$files = "";
				if( $default['corp_id']!="")
					$files .= "/" . $default['corp_id'];
				if( $default['document_group']!="")
					$files .= "/" . $default['document_group'];	
				$files .= "/" . $default['file_name'];
				
				$file = $file . "/" . $files;
				if (file_exists($file)) {
					header("Location: /files/edocument/" . $files );
					exit;
					...
					echo "<br />" . $default['document_group']  . "/" . $default['file_name'];
					die();
				}
