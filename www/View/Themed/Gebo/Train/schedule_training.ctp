...
<table class="table table-bordered tablesorter" style="margin-top: 10px;">
    ...
    <tbody>
      ...
      foreach($data as $row){
          ...
          ";
          if($row[0]['base']=="base"){
              $fromDate = date("d-m-Y",strtotime($row[0]['fromdatebase']));
              $toDate = date("d-m-Y",strtotime($row[0]['todatebase']));
              echo "
                  <td>
          ...
          if($row[0]['base']=="outbase"){
              $fromDate = date("d-m-Y",strtotime($row[0]['fromdateoutbase']));
              $toDate = date("d-m-Y",strtotime($row[0]['todateoutbase']));
