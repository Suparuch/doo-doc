...
class HistoryOthersController extends AppController {

    public $name = 'HistoryOthers';
    public $uses = array('HistoryOther', 'Unit', 'TableAttachFile');
    ...
    public function index() {
        ...
        if (empty($dataRequest)) {
            ...
            $total = $this->HistoryOther->find('count', $conditions);
            $HistoryOthers = $this->HistoryOther->find('all', $conditions);
        } else {
            ...
            if (!empty($dataRequest['txtHoName'])) {
                $whereConditions[] = '(HistoryOther.ho_name like \'%' . trim($dataRequest['txtHoName']) . '%\' )';
            }
            if (!empty($dataRequest['txtHoFormat'])) {
                $whereConditions[] = '(HistoryOther.ho_format like \'%' . trim($dataRequest['txtHoFormat']) . '%\' )';
            }
            if (!empty($dataRequest['txtHoLocation'])) {
                $whereConditions[] = '(HistoryOther.ho_location like \'%' . trim($dataRequest['txtHoLocation']) . '%\' )';
            }
            ...
            // Query
            $total = $this->HistoryOther->find('count', array('conditions' => $whereConditions));
            $HistoryOthers = $this->HistoryOther->find('all', $conditions);
        }
        ...
        $this->set('HistoryOthers', $HistoryOthers);
        $pagination = array('offset' => $offset, 'total' => $total, 'limit' => $this->Generator->setLimit(), 'model' => 'HistoryOther', 'pages' => 5);
    ...
    public function form($id = null) {
        ...
        if (!empty($id)) {
            ...
            foreach ($HistoryOthers[0]['HistoryOther'] as $key => $value)
                $default[$key] = $value;
    ...
    public function save($id = null, $formAction = "EDIT") {
        ...
        if (!empty($data)) {
            if (!empty($id)) {
                $this->HistoryOther->id = $id;
                if ($formAction == "ADD") {
                    $data['id'] = $id;
                    $this->HistoryOther->create();
                }
                $status = $this->HistoryOther->save($data) ? 'SUCCESS' : 'FAILED';
            }
        } else
            $status = 'FAILED';
                    ...
                    if (is_uploaded_file($_FILES['Filedata']['tmp_name'])) {
                        if ($_FILES['Filedata']['error'] > 0) {
                            ...
                            if ($uploadSuccess) {
                                ...
                                $document['file_table_name'] = 'HistoryOthers';
