...
class AreasController extends AppController {
	...
	public function index() {
		...
		if(empty($dataRequest)){
			...
			$total = $this->Area->find('count',$conditions);
			...
			$total = $this->Area->find('count', array('conditions' => $whereConditions ));
			$Areas = $this->Area->find('all',$conditions); 
		...
		$pagination = array('offset' => $offset , 'total' => $total , 'limit' => $this->Generator->setLimit(), 'model' => 'Area','pages' => 5);
	...
	public function form($id = null) {
		...
		if(!empty($id)){
				...
				foreach($Areas[0]['Area'] as $key => $value)
					$default[$key] = $value;
	...
	public function save($id=null) {
		...
		if(!empty($data)){    
			...
			if(!empty($id)){
				...
				$this->Area->id = $id;
				if($this->Area->exists())
				$status = $this->Area->save($data) ? 'SUCCESS' : 'FAILED'; 
				else $status = 'FAILED';    
