...
class HMReportsController extends AppController {

    public $name = 'HMReports';
    public $uses = array('HMReport', 'Unit', 'TableAttachFile');
    ...
    public function index() {
        ...
        if (empty($dataRequest)) {
            ...
            $total = $this->HMReport->find('count', $conditions);
            $HMReports = $this->HMReport->find('all', $conditions);
        } else {
            ...
            // Search condition
            // 
            if (!empty($dataRequest['txtRpTitle'])) {
                $whereConditions[] = '(HMReport.rp_title like \'%' . trim($dataRequest['txtRpTitle']) . '%\' )';
            }
            if (!empty($dataRequest['ddlYear'])) {
                $whereConditions[] = '(HMReport.rp_year = \'' . trim($dataRequest['ddlYear']) . '\' )';
            }
            ...
            // Query
            $total = $this->HMReport->find('count', array('conditions' => $whereConditions));
            $HMReports = $this->HMReport->find('all', $conditions);
        }
        ...
        // Set model to view
        $this->set('HMReports', $HMReports);
        $pagination = array('offset' => $offset, 'total' => $total, 'limit' => $this->Generator->setLimit(), 'model' => 'HMReport', 'pages' => 5);
    ...
    public function form($id = null) {
        ...
        if (!empty($id)) {
            ...
            foreach ($HMReports[0]['HMReport'] as $key => $value)
                $default[$key] = $value;
    ...
    public function save($id = null, $formAction = "EDIT") {
        ...
        if (!empty($data)) {
            if (!empty($id)) {
                ...
                if ($formAction == "ADD") {
                    $data['id'] = $id;
                    $this->HMReport->create();
                }
                $status = $this->HMReport->save($data) ? 'SUCCESS' : 'FAILED';
            }
        } else
            $status = 'FAILED';
                    ...
                    if (is_uploaded_file($_FILES['Filedata']['tmp_name'])) {
                        if ($_FILES['Filedata']['error'] > 0) {
                            ...
                            if ($uploadSuccess) {
                                ...
                                $document['file_table_name'] = 'HMReports';
