...
body, input, textarea { font-family:TH SarabunPSK; font-size:9pt; }
...
$header = '
		...
		<tr>
			...
			<td width=180px align="right">'.$model_name.'</td>
		</tr>
		</table>
		...
		<table border ="0">
			<thead>
				<tr class="border_top_header">
					...
					<th rowspan="2" width=263px>รายการยุทโธปกรณ์</th>
 ...
 $sum = array(0,0,0,0,0);
  ...
  if($i<10){ $j= "0".($i+1);}else{$j=$i; }
	...
	<tr>
		...
		<td >&nbsp;</td>
		<td class="left" colspan="7">'.$Tech['Tech']['name'].'</td>
	</tr> 
	';
	...
	foreach($ModelDivisions[$i] as $ModelDivision){
		...
		$quantity = $ModelDivision['ModelGroup']['quantity'];
		...
		$body .= '<tr>
			...
			$body .= '			
				...
				<td class="center">'.($ModelDecrease['rate_decrease_3']*$quantity).'</td>
...
if($m !=1){
	$body .= '<tr>
		...
		<td class="center"><b>'.number_format($sum[3]).'</b></td>
		...
		<td></td>
		<td></td>
		<td class="left" colspan="8">(ไม่กำหนดยุทโธปกรณ์)</td>
	</tr> ';
	...
	<td class="center"><b>'.number_format($sumTotal[3]).'</b></td>
