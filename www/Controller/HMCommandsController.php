...
class HMCommandsController extends AppController {

    public $name = 'HMCommands';
    public $uses = array('HMCommand', 'Unit', 'TableAttachFile');
    ...
    public function index() {
        ...
        if (empty($dataRequest)) {
            ...
            $total = $this->HMCommand->find('count', $conditions);
            $HMCommands = $this->HMCommand->find('all', $conditions);
        } else {
            ...
            // Search condition
            // 
            if (!empty($dataRequest['txtCmNo'])) {
                $whereConditions[] = '(HMCommand.cm_no like \'%' . trim($dataRequest['txtCmNo']) . '%\' )';
            }
            if (!empty($dataRequest['txtCmTitle'])) {
                $whereConditions[] = '(HMCommand.cm_title like \'%' . trim($dataRequest['txtCmTitle']) . '%\' )';
            }
            if (!empty($dataRequest['dpkCmDate'])) {
                //$dCmdate = $this->TextUtil->formatPckDateForWhereCond($dataRequest['dpkCmDate']);
                
                $dCmdate = $this->TextUtil->formatDateForWhereCond($dataRequest['dpkCmDate']);
                if(!empty($dCmdate)){
                    $whereConditions[] = '(HMCommand.cm_date =\'' . $dCmdate . '\' )';
                }else{
                    $default['dpkCmDate']['day'] = '';
                    $default['dpkCmDate']['month'] = '';
                    $default['dpkCmDate']['year'] = '';
                }
            }
            ...
            // Query
            $total = $this->HMCommand->find('count', array('conditions' => $whereConditions));
            $HMCommands = $this->HMCommand->find('all', $conditions);
        }
        ...
        $breadcrumb = array(
            'controller' => array(
                ...
                'url' => '',
                'subtitle' => 'คำสั่ง/คู่มือการปฏิบัติงาน'
            ),
        );
...
//        $Units = $this->Unit->find('list', $conditions);
        ...
        $this->set('HMCommands', $HMCommands);
        $pagination = array('offset' => $offset, 'total' => $total, 'limit' => $this->Generator->setLimit(), 'model' => 'HMCommand', 'pages' => 5);
    ...
    public function form($id = null) {
        ...
        if (!empty($id)) {
            ...
            foreach ($HMCommands[0]['HMCommand'] as $key => $value)
                $default[$key] = $value;
...
//        $Units = $this->Unit->find('list', $conditions);
    ...
    public function save($id = null, $formAction = "EDIT") {
        ...
        if (!empty($data)) {
            if (!empty($id)) {
                $this->HMCommand->id = $id;
                if ($formAction == "ADD") {
                    $data['id'] = $id;
                    $this->HMCommand->create();
                }
                $status = $this->HMCommand->save($data) ? 'SUCCESS' : 'FAILED';
            }
        } else
            $status = 'FAILED';
                    ...
                    if (is_uploaded_file($_FILES['Filedata']['tmp_name'])) {
                        if ($_FILES['Filedata']['error'] > 0) {
                            ...
                            if ($uploadSuccess) {
                                ...
                                $document['file_table_name'] = 'HMCommands';
