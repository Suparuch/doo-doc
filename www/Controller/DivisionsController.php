...
class DivisionsController extends AppController {
	...
	public function index() {
		...
		if(empty($dataRequest)){
			...
			$total = $this->Division->find('count',$conditions);
			$Divisions = $this->Division->find('all',$conditions); 
			...
			$total = $this->Division->find('count', array('conditions' => $whereConditions ));
			$Divisions = $this->Division->find('all',$conditions); 
		...
		$pagination = array('offset' => $offset , 'total' => $total , 'limit' => $this->Generator->setLimit(), 'model' => 'Division','pages' => 5);
	...
	public function form($id = null) {
		...
		if(!empty($id)){
				...
				foreach($Divisions[0]['Division'] as $key => $value)
					$default[$key] = $value;
		...
		$data = $this->request->data;    
			...
			if(!empty($data)){    
				...
				if(!empty($id)){
					...
					$status = $this->Division->save($data) ? 'SUCCESS' : 'FAILED'; 
					else $status = 'FAILED';    
